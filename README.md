...
Some URLs cannot be checked automatically, e.g. a link to a private repository,
or a page behind a login or captcha. List them in a `.urlignore` file (in the
project root, or under `tools/`) to stop `url_check()` from requesting and
flagging them. Each line is a glob pattern:
