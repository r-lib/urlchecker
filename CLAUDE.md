...
Public API:
[`url_check()`](https://urlchecker.r-lib.org/reference/url_check.md) and
[`url_update()`](https://urlchecker.r-lib.org/reference/url_update.md).

## Architecture

The core URL-checking logic is **not written here** — it is a verbatim
copy of R’s `tools` package code, embedded at
[inst/tools/urltools.R](https://urlchecker.r-lib.org/inst/tools/urltools.R).

- [R/zzz.R](https://urlchecker.r-lib.org/R/zzz.R) `.onLoad()` sources
  `urltools.R` into an environment `tools` whose parent is
...
- The package’s own code wraps that embedded logic:
  - [R/url_check.R](https://urlchecker.r-lib.org/R/url_check.R) —
  ...
  - [R/rmd.R](https://urlchecker.r-lib.org/R/rmd.R) — checks URLs in
    un-rendered Rmd vignettes (needs pandoc).
  - [R/url_update.R](https://urlchecker.r-lib.org/R/url_update.R) —
    rewrites permanently-redirected (301) URLs in place.
  - [R/utils.R](https://urlchecker.r-lib.org/R/utils.R) —
...
[MAINTENANCE.md](https://urlchecker.r-lib.org/MAINTENANCE.md)).
...
2.  If the new code calls an internal `tools` function not present in
    ...
    `tools` env from [R/zzz.R](https://urlchecker.r-lib.org/R/zzz.R).
    (This is how `lines2str` was once handled, before it became
    available in the installed `tools` namespace for all supported R
    versions.)
3.  Keep this file OUT of Air formatting — it must stay in upstream
    ...
    [air.toml](https://urlchecker.r-lib.org/air.toml). (Air’s `exclude`
    key lives under `[format]`, not top level; the IDE’s TOML schema may
    wrongly flag it, but Air itself honors it.)
...
- Tests: [`uncovr::test()`](https://rdrr.io/pkg/uncovr/man/test.html).
  The suite lives in
  [tests/testthat/](https://urlchecker.r-lib.org/tests/testthat/) and
  uses testthat 3e. It avoids the network by running a local `webfakes`
  app
  ([helper-webfakes.R](https://urlchecker.r-lib.org/tests/testthat/helper-webfakes.R))
  ...
  [tests/testthat/\_snaps/](https://urlchecker.r-lib.org/tests/testthat/_snaps/).
...
- Formatting: `air format .` (uses
  [air.toml](https://urlchecker.r-lib.org/air.toml); leaves `urltools.R`
  untouched).
