...
function(dir)
{
    ...
    for (rfile in rfiles) {
      if(!is.na(rfile) && nzchar(Sys.which("pandoc"))) {
        rpath <- tools$.file_path_relative_to_dir(rfile, dir)
        tfile <- tempfile(fileext = ".html")
        on.exit(unlink(tfile), add = TRUE)
        out <- tools$.pandoc_md_for_CRAN2(rfile, tfile)
        if(!out$status) {
...
.pandoc_md_for_CRAN2 <- function(ifile, ofile) {
    tools$.system_with_capture("pandoc", paste(shQuote(normalizePath(ifile)),
